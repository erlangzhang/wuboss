        ...
        <div class='table-container'>
            ...
            <el-dropdown class="select-row-dropdown" :hide-on-click="false">
                ...
                <el-dropdown-menu slot="dropdown">
                    ...
                    <el-dropdown-item>
                        <el-checkbox v-model="columnsChecked.operator">上传人</el-checkbox>
                    </el-dropdown-item>
                    <el-dropdown-item>
                        <el-checkbox v-model="columnsChecked.uploadTime">上传时间</el-checkbox>
                    </el-dropdown-item>
                    <el-dropdown-item>
            ...
            <el-table :data="datas" v-loading="loading" element-loading-text="加载中" border style="width: 100%" @sort-change='handleSortChange' @filter-change="getChargeType">
                ...
                <el-table-column prop="category" label="分类" :filter-multiple="false" v-if="columnsChecked.class" width="100" :filters="categoryList" :filter-method="filterCategory" filter-placement="bottom-end" column-key="category">
                </el-table-column>
                ...
                <el-table-column prop="operatorName" label="上传人" v-if="columnsChecked.operator" width="180">
                </el-table-column>
                <el-table-column prop="operatorTime" label="上传时间" v-if="columnsChecked.uploadTime" width="180" :sortable="true">
                </el-table-column>
                <el-table-column label="操作" v-if="columnsChecked.opt" show-overflow-tooltip>
                    <template scope="scope">
                        <router-link class="retrieve-btn" :to="'/content/unchecked-detail?id='+scope.row.id">
                            <VIcon type='info-circle-o' class='iconfont-button' />进入审核</router-link>
                    </template>
                </el-table-column>
            </el-table>
    ...
    data() {
            ...
            getLastPage: function() {
                ...
                if (yushu != 0) {
                    if (this.pagination.currentPage <= maxCurrPage) {
                        console.log("能整出的页数>>>", maxCurrPage, "当前页>>>", this.pagination.currentPage);
                        return this.pagination.currentPage * this.pagination.pageSize;
                    } else {
                        console.log("当前页已经是最后一页");
                        return this.pagination.total;
                    }
                } else {
                    return this.pagination.currentPage * this.pagination.pageSize;
        ...
        methods: {
            ...
            fetch() {
                ...
                this.loading = true;
                contentStore.listUncheckMerchantInfo(params).then(res => {
                    ...
                    this.datas.map(item => {
                        ...
                        item.operatorName = item.upload.operatorName;
                        item.operatorTime = moment.unix(item.upload.uploadTime).format("YYYY-MM-DD HH:mm:ss");
                    });
                    this.loading = false;
                }).catch(err => {
