    ...
    <div class='layout-container'>
        <div class='breadcrumb'>
            <el-breadcrumb separator=">">
                ...
                <el-breadcrumb-item>栏目库</el-breadcrumb-item>
            </el-breadcrumb>
            <div class='breadcrumb-header'></div>
        </div>
        <div class='content-header'>
            <VIcon type='appstore' /> 栏目库(内容库)
            <div class='breadcrumb-header'></div>
        </div>
        <div class='operator-bar'>
            <router-link class="create-btn" to='/content/new-item'>
        ...
        <div class='table-container'>
            ...
            <el-table :data="datas" v-loading="loading" element-loading-text="加载中" border style="width: 100%" @sort-change='handleSortChange' @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="updatedAt" sortable label="更新时间" width="180" v-if="columnsChecked.updatedAt">
                    <template scope="scope">
                        <label> {{ renderTime(scope.row.updatedAt) }}</label>
                    </template>
                </el-table-column>
                <el-table-column label="操作" v-if="columnsChecked.opt" width="180" show-overflow-tooltip>
                    <template scope="scope">
                        <router-link class="edit-btn" :to="'/content/new-item?id=' + scope.row.columnId">
                ...
                contentStore.listItem(params).then(ret => {
                    ...
                    for (const item of this.datas) {
                        ...
                        for (const itemS of this.selectItems) {
                            if (item.columnId === itemS.columnId) {
                    ...
                    this.$message.error(`列表栏目数据失败！${err}`);
                    this.loading = false;
                });
            },
            ...
            handleSelectionChange(selection) {
                for (const item of this.datas) {
                    ...
                    for (const item1 of this.selectItems) {
                        if (item1.columnId === item.columnId) {
