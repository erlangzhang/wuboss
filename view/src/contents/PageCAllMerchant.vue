        ...
        <div class='table-container'>
            ...
            <el-dropdown class="select-row-dropdown" :hide-on-click="false">
                ...
                <el-dropdown-menu slot="dropdown">
                    ...
                    <el-dropdown-item>
                        <el-checkbox v-model="columnsChecked.putinTime">入库时间</el-checkbox>
                    </el-dropdown-item>
                    <el-dropdown-item>
            ...
            <!--    <el-input class='search-input' placeholder="商户名称/ID" icon="search" v-model="search" size="small" :on-icon-click="handleSearch" @input="enterToSearch($event)">
            </el-input> -->
            <SearchInput :defaultSearch="defaultSearch" :handleSearch="handleSearch"></SearchInput>
            <el-table :data="datas" v-loading="loading" element-loading-text="加载中" border style="width: 100%" @sort-change='handleSortChange' @filter-change="getFilterCondition">
                ...
                <el-table-column prop="merchantDist" label="地区" class="province-style" v-if="columnsChecked.region" width="100" :filter-multiple="false" :filters="provinceList" :filter-method="filterProvince" filter-placement="top" column-key="merchantDist"></el-table-column>
                <el-table-column prop="category" label="分类" v-if="columnsChecked.class" width="100" :filter-multiple=false :filters="categoryList" :filter-method="filterCategory" filter-placement="bottom-end" column-key="category">
                </el-table-column>
                ...
                <el-table-column prop="status" label="状态" v-if="columnsChecked.status" width="100" :filters="statusList" :filter-multiple=false :filter-method="filterStatus" filter-placement="bottom-end" column-key="status">
                    <template scope="scope">
                        <el-tag style="margin-top: 5px;">{{renderStatus(scope.row.status) }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="onlineTime" label="入库时间" v-if="columnsChecked.putinTime" width="180" :sortable="true">
                </el-table-column>
                <el-table-column label="操作" v-if="columnsChecked.opt" show-overflow-tooltip>
                    <template scope="scope">
                        <router-link class="retrieve-btn" :to="'/content/all-merchant-detail?id='+scope.row.id">
                            <VIcon type='info-circle-o' class='iconfont-button' />查看详细</router-link>
                        <router-link class="edit-btn" :to="'/content/add-merchant?id=' + scope.row.id">
            ...
            <el-dialog v-model="dialogVisible" size="tiny">
                <span><VIcon type="question-circle" class="delete-btn-dialog-icon"/>确定删除吗？
                </span>
...
export default {
    data() {
        ...
        methods: {
            ...
            handleDelOk() {
                contentStore.deleteMerchantInfo(this.selectRow.id).then(ret => {
            ...
            fetch() {
                ...
                this.loading = true;
                contentStore.listMerchantInfo(params).then(res => {
                    ...
                    this.datas.map(item => {
                        ...
                        item.onlineTime = moment.unix(item.upload.uploadTime).format("YYYY-MM-DD HH:mm:ss");
                    });
                    this.loading = false;
                }).catch(err => {
